...
def I_ball(p, kF):
    ...
    # I_ball = 4pi x 2pi kF (1 + tL) = 8 pi^2 kF (1 + tL); pinned by BOTH limits:
    # p->0 gives 16 pi^2 kF (direct: 4pi x 4pi kF) and p=kF gives 8 pi^2 kF.
    # (Construction iterations 4-5: the X8-zero gate caught first a missing 2pi,
    # then the bracket written as its own average — exact factors 1/2pi and 1/2.)
    return 8*math.pi**2*kF*(1.0 + t*L)
