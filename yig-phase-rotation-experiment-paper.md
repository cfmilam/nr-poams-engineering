...
The critical, robust prediction: the sign of the weight change reverses when the alignment field polarity is reversed. Two channels set that sign, with *opposite* co-sense signs — the static DC alignment deposits **twist** (co-spin heavier, the Barnett/brass-gyroscope sign), while the coherent FMR drive pumps **writhe** (orbital winding; co-sense lighter, forward-derived). Toggling the drive at fixed alignment isolates the drive/writhe contribution, whose sign reads which channel the coherent phase-advance feeds — the λ-partition; either sign is a positive result. No conventional mechanism predicts a directional, reversible weight change at FMR. Detection of this signature at milligram scale or above constitutes direct experimental confirmation of POAMS.
...
The reason nobody followed the thread is structural. The people who understood FMR and magnon physics did not believe in gravitational anomalies. The people who believed in gravitational anomalies did not understand FMR and magnon physics. These two communities never overlapped.

The separation was enforced, not accidental. Eric Laithwaite — father of maglev, inventor of the linear induction motor, the one man of his era whose hands knew both electromagnetic machines and gyroscopes — used his 1974 Royal Institution discourse to demonstrate how strangely spinning bodies handle and to say aloud that Newton's foundation might be the problem. The response was institutional annihilation: the discourse text was refused publication in the Institution's Proceedings, his Royal Society fellowship nomination was cancelled, his Royal Institution professorship ended, and he spent his remaining career in exile — NASA awarded his electromagnetic launch-assist proposal weeks before his death in 1997. He had overclaimed: a precessing rotor redistributes torque, it does not statically lighten, and he later conceded the motions reconcile with mechanics. But the punishment was calibrated to the heresy, not to the error, and every career-minded physicist took the lesson: spin-plus-gravity is professional death. That is how the two communities that needed to meet were kept apart for the next half century.

Alzofon came closest — he was at Boeing, he understood microwave physics, he designed a real experiment — but he wrapped the correct mechanism in incorrect theoretical ontology, and the physics community dismissed him accordingly.
...
**On the sign — stated in Normal Realist terms.** It is essential here not to smuggle back in the container picture. There is no spatial "field" surrounding the Earth that the body sits *inside* and that "thins" with distance. In Normal Realism there is no empty stage on which the angular momentum is arranged: **the angular momentum *is* the happening** — it *is* the spatial separation, the mass, and the motion, all at once (Pope & Osborne). "Radius" is not a distance across an empty gap; it is the angular-momentum relation between the body and the Earth, expressed as a length-measure. So the sign of the weight change is not a fact about *where the body is in a field*; it is a fact about *how two angular momenta compose*.

The relevant composition is Pope's, from *Einstein's Lost Legacy*, Appendix 4/5. Angular momentum is a vector; the body's internal spin angular momentum and its orbital angular momentum (its co-rotation with the Earth) **add and subtract like velocities** — magnitudes summing when the vectors are aligned, differing when opposed, with the general case following the standard 0°–180° law of vector composition. The total spin-plus-orbital angular momentum is what fixes the natural orbital radius via `L = mvr`; equivalently, spin is absorbed into a variable `G` that equals the textbook constant only for a spinless body. Pope states the result for exactly our case:

> *"adding spin angular momentum to a particle, say, on the Earth's equator, depending on its direction of travel, will increase or decrease its natural orbital speed relative to the Earth's surface, **making it weigh either more or less, depending on the direction of its motion**."* (Appendix 5)

And the direction rule, from Appendix 4/5: when the spin and orbital angular-momentum vectors are **in the same sense**, the natural radius **contracts** — the body's fixed surface radius now *exceeds* its (contracted) natural radius, equivalently its natural orbital speed relative to the Earth's surface increases, so it bears harder on the surface constraint → **heavier**. When **opposed**, the natural radius **extends** past the surface (natural orbital speed decreases) → the contact reaction drops → **lighter**. (The "weight" throughout is the contact reaction at the surface, not any *in vacuo* force acting across a gap — there is no such force in Normal Realism.) This is our co-spin/anti-spin result — and it is Pope's own derivation, not a new posit.

Note the counterintuition this resolves: a naive reading of `L = m√(GMr)` suggests that more angular momentum should mean a *larger* radius. Pope's variable-`G` composition is precisely what inverts that — added co-sense spin folds into an effectively increased `G`, contracting the natural radius rather than expanding it. The sign that feels backwards is the correct one.

*Labeling note (the right-hand rule).* We name vectors by the right-hand rule, so "co-spin" = the body's spin angular-momentum vector in the same sense as its orbital co-rotation with the Earth. This is pure labeling and carries no physics: a left-hand rule would relabel both vectors together, leaving their composition — and the result — unchanged. The sign lives entirely in the *physical direction of the spin vector relative to the orbital vector*, which is a measurable angular-momentum relation, not a convention and not an assumption about a surrounding medium. The experiment measures that relation directly. (Note for reviewers: an independent formal re-derivation once reached the opposite sign — not by disputing any physical fact, but by composing the two angular-momentum vectors in the reversed sense. The lesson is that the sign is fixed by the actual vector composition per Appendix 4/5, and must be read off Pope's derivation rather than re-assumed.)

**The alignment/drive split (the λ-partition) — forward-derived 2026-07-19 (Opus + Sol, cross-vendor, independent).** The sign above is the *alignment* channel: co-orienting the internal spins deposits net intrinsic spin — twist — which composes into the effective coupling (variable `G`, App. 4/5) and, co-sense, contracts the natural radius → heavier. This is the settled, brass-gyroscope-confirmed case. The FMR *drive* is a distinct deposit: it cannot change any vortex's constitutive own-axis spin, so it can only advance orientation, and a spin axis coning through angle θ sweeps solid angle Ω = 2π(1-cosθ) — i.e. it pumps **writhe**, not twist. Writhe adds to the orbital column `L_orb` and, co-sense, *extends* the natural radius → **lighter** — a bounded offset ∝(1-cosθ), held only while driven and relaxing when it stops. Alignment (twist) and drive (writhe) therefore carry *opposite* co-sense signs. The leading/lagging phase-slip account below is written in this **writhe reading** (leading/co-sense → lighter), matching the paper's own chain (more `L_orb` → larger radius); the static-alignment (twist) channel keeps the opposite co-sense sign (heavier), as measured in the brass/bismuth precedents. Which channel the FMR drive actually feeds is the λ-partition, and the clean discriminant is the **drive toggled on/off at fixed alignment**: its sign reads the partition directly — both signs are a positive result. This rests on the driven coning being physical axis-winding rather than a relabeling of a precessing spin, decided empirically by whether cutting the drive leaves a persistent added linking (the weight-relaxation measurement). [Forward derivation: `memory/poams-audit/LAMBDA-SWAMP-forward-opus.md`; sign reconciliation: `directional-sign-2026-07-19.md`.]
...
- **Directional** (the sign reverses with alignment polarity): the static-alignment/twist contribution is same-sense → heavier (Pope, App. 4/5; the brass/bismuth sign), while the coherent FMR-drive/writhe contribution is opposite (same-sense → lighter). Which dominates the measured sign is the λ-partition the drive-toggle reads — see the alignment/drive split above.
...
- **Leading slip:** The collective vector advances faster than the reference frame → net angular momentum addition in the co-rotation sense. This driven advance is a coning of the axis — swept solid angle — which is **writhe** (orbital winding); it adds to the orbital column `L_orb`, and since `r_ff ∝ L_orb²` the natural radius **extends** → **lighter**.
- **Lagging slip:** The collective vector advances slower than the reference frame → writhe removed from `L_orb` → the natural radius **contracts** → **heavier**.
...
4. **The experiment changes L without changing the material.** Phase advance rotates the collective angular momentum vector without adding ħ to any individual vortex (which would be transmutation). The atoms remain iron. The ensemble gains net angular momentum — specifically **writhe** (the coning axis sweeps solid angle), which adds to the orbital column `L_orb`. Consistent with points 1–3 (more `L_orb` → larger natural radius), a co-sense gain **extends** the radius → **lighter**; an anti-sense gain **contracts** it → **heavier**. This is the *opposite* co-sense sign to the static DC-alignment (twist) channel above — the two are the λ-partition, and the drive toggled on/off at fixed alignment isolates this driven/writhe contribution.
...
| Laithwaite | 1974 | Precessing gyroscopes | Steel/brass rotors | Mechanical rotation only (Barnett ~10⁻¹⁰) | Forced precession | Bulk rotation — weakest spin lever, fully swamped | Vivid demonstrations; no static weight change survived weighing; career destroyed |
...
- **Laithwaite** demonstrated the phenomenology, not the mechanism. A precessing rotor *handles* differently — every audience could see it — but bulk mechanical rotation is the weakest possible lever on internal spin state: Barnett alignment at his rotor speeds is parts in 10¹⁰, and the swamping analysis (§3) says exactly why his tabletop showed nothing a balance could keep. His ontological instinct was correct — spin is a first-class dynamical entry, and the rectilinear-inertia default beneath Newton's laws is the wrong foundation; that instinct is POAMS §3.1 stated fifty years early, without the framework. He was one conceptual step from the right lever: his own linear motors phase-advance circulation electromagnetically — applied to internal spins instead of rotors, that is this experiment. The institutional destruction he absorbed (§2.3) is why the step was never taken.
...
|---|---|
| Weight change that reverses with field polarity | Polarization (alignment/twist) alone couples to the constraint force — co-spin heavier, the Barnett/brass sign. The FMR drive adds the *writhe* channel (opposite co-sense sign); the drive-on/off change at fixed polarity isolates it. |
...
- Containment tube capped at both ends — one alignment sense drives the sphere lighter (it lifts), the other heavier (it presses down harder); which sense is which is the sign the experiment measures. Both directions require containment.
