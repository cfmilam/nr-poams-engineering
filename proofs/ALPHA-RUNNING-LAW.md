...
forward from Normal-Realist ground truth, and an independent **cross-vendor** formalizing
adversary hunting the hidden assumption. Every step pre-registered — stated before any comparison
to the measured running curve. A distinctive prediction is counted a success; a fitted match a failure.

## The question
Part II proved α's *value* is a flat direction (contingent, by theorem). A distinct question:
does the theory fix how α *changes with scale* — its running? Conventionally α⁻¹ falls from
137.036 at atomic scales toward ≈128 near the Z mass, and quantum electrodynamics reads that fall
as α⁻¹ **linear in** ln(energy). Pre-register the POAMS form before touching that curve.

## Ground truth used (forward only)
- Time-cone scale-invariance (Part I).
- β = v/c = √(2/N), the winding tilt; β is the **marginal flat direction** (eigenvalue exactly 1) of §12.
- Nested structure: inner spin (scale r_s = ħ/2mc) wrapped by outer orbital (scale r_orb = ħ/mβc),
  so r_orb/r_s = 2/β; the inner spin is the coherent, rigid, protected bulk, the outer orbital the responder.
- The probe resolves a length ℓ = ħ/Q (finer probe = smaller ℓ = deeper into the winding).
- Closure Wβ² = 4 ⇒ W = 4/β², α⁻¹ = ½√W.

## What POAMS FORCES (banked as theorem)
1. **Sign** — α⁻¹ falls as the probe resolves deeper (finer probe ⇒ larger tilt ⇒ smaller α⁻¹).
2. **Marginality** — the running **freezes** at the flat-direction fixed point (the eigenvalue-1 mode
   of the collective bootstrap); there is no leading running. Structural, not fitted.
3. **Thresholds** — the running turns on as each nested circulation becomes resolvable on the mc²/h ladder.
